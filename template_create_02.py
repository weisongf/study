...
def create_configfile(role_type, enable_ha="no", manage_ip, hostname,ntp_server_ip):
    # 读取配置文件
    ...
    if enable_ha == "no":
        ...
        if role_type == "controller":
            ...
            try:
                config.add_section("ha")
                config.set("ha", "enable_ha", "yes")
                config.set("ha", "controller_nums", "3")
                config.set("ha", "controller_members", "172.200.4.11")
                config.set("ha", "controller_internal_vip", "172.200.4.10")
            except ConfigParser.DuplicateSectionError:
                print("Section 'ha' already exists")
                elif role_type == "compute":
                try:
